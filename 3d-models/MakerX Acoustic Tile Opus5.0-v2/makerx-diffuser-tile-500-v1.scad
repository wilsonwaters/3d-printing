// === DESCRIPTION ===
// MakerX Acoustic Diffuser — 500 SERIES (tile-500-v1). An ALTERNATE to the 125mm
// joinable quadrant (makerx-diffuser-tile-v2.scad), not a replacement. Both are
...
// separately printed KEY piece fills. Each tile's main diagonal is one arm of a single
// 500mm MakerX X, and the key is its crossing point.
//
// WHY THE NOTCH
...
//   "the X" / "arm"               -> x_cell(), skin_group(), SKIN_COLOURS
//   "colour skin"                 -> skin_t, cell_skin(), part="skin"/"body"
//   "cell"/"block"/"cavity"/"neck"-> as in makerx-diffuser-tile-v2.scad
//
// Common modifications:
//   Zero-purge colour instead     -> colour_mode="elevation" (X becomes the tallest
//                                    cells; height field picks up a diagonal ridge)
...
colour_mode = "skin";        // "skin" (per-cell top skin) | "elevation" (bands)
skin_group  = 0;             // which colour to emit when part="skin" (0..2)
...
SKIN_COLOURS = [MX_GOLD, MX_MAGENTA, MX_EBLUE];   // inward -> outward along the arm
...
// "elevation" mode needs the X to BE the tallest cells; "skin" mode does not care.
function height_index(x, y) =
    colour_mode == "elevation" && x_cell(x, y) ? 6 : mixed_s(x, y);

function block_h(x, y) = height_index(x, y) * depth_step;
function in_notch(x, y) = (x < notch_cells) && (y < notch_cells);

// The X: the MAIN diagonal only. Under the assembly rotations these four diagonals form
// one 500mm X radiating from the centre; the anti-diagonal would draw a second X inside
// every tile.
function x_cell(x, y) = (x == y) && !in_notch(x, y);

// Colour group along the arm: 0 gold (innermost, by the key) .. 2 electric blue (tip).
function skin_group_of(x, y) =
    !x_cell(x, y) ? -1
  : let (span = grid_n - notch_cells, k = floor((x - notch_cells) * 3 / span))
    min(k, 2);
...
x_cells = len([ for (y = [0:grid_n-1], x = [0:grid_n-1]) if (x_cell(x, y)) 1 ]);
...
echo(str("=== MakerX 500 series: ", grid_n, "x", grid_n, " cells, colour_mode=",
         colour_mode, " ==="));
...
echo(str("  X cells ", x_cells, " on the main diagonal; colour = top ", skin_t, " mm"));
...
module skin_boxes(group) {
    for (y = [0:grid_n-1], x = [0:grid_n-1])
        if (skin_group_of(x, y) == group) skin_box(x, y);
}

module tile_skin(group) { intersection() { tile_solid(); skin_boxes(group); } }
module tile_body()      { difference()   { tile_solid();
                                           for (g = [0:2]) skin_boxes(g); } }
...
module key_body()     { difference()   { key_solid(); key_skin_boxes(); } }
module key_skin()     { intersection() { key_solid(); key_skin_boxes(); } }
module key_coloured() { color(MX_NAVY) key_body(); color(MX_GOLD) key_skin(); }
...
module assembly(coloured = false) {
    for (a = ASSEMBLY)
        translate([a[0][0] * tile_size, a[0][1] * tile_size, 0])
            translate([tile_size / 2, tile_size / 2, 0]) rotate([0, 0, a[1]])
                translate([-tile_size / 2, -tile_size / 2, 0])
                    if (coloured) tile_coloured(); else tile();
    translate([tile_size - notch_w + key_gap, tile_size - notch_w + key_gap, 0])
        if (coloured) key_coloured(); else key();
}

module tile_coloured() {
    color(MX_NAVY) tile_body();
    for (g = [0:2]) color(SKIN_COLOURS[g]) tile_skin(g);
}
...
else if (part == "tile_colour")     tile_coloured();
else if (part == "body")            tile_body();
else if (part == "skin")            tile_skin(skin_group);
else if (part == "key_body")        key_body();
else if (part == "key_skin")        key_skin();
