...
module gate_latch_lever() {
    difference() {
        union() {
            // Main lever arm - extended to accommodate separated holes
            hull() {
                translate([-10, 0, 0])  // Extended 10mm to the left
                    cube([lever_length - handle_diameter/2 + 10, lever_thickness, lever_width]);
            ...
            translate([lever_length - handle_diameter/2, lever_thickness/2, lever_width/2])
                rotate([90, 0, 0])  // Original orientation
                    cylinder(d=handle_diameter, h=handle_thickness, center=true);  // 5mm height
            
            // 25mm cylindrical extension at connection end for latch connector (one-sided)
            translate([-10, lever_thickness, lever_width/2])
                rotate([90, 0, 0])
                    cylinder(d=8, h=25);  // 25mm long cylinder, 8mm diameter, extending from lever surface
            
            // Symmetrical ramped joins on both sides for strength
            hull() {
                translate([-10, lever_thickness-0.1, 0])
                    cube([8, 0.1, lever_width]);  // Base connection to full lever width
                translate([-10, lever_thickness - 5, lever_width/2])
                    rotate([90, 0, 0])
                        cylinder(d=8, h=0.1);  // Cylinder connection point
            }
            
            // Reinforcement at pivot point (critical for ASA strength)
            translate([15, lever_thickness/2, lever_width/2])
                rotate([90, 0, 0])
                    cylinder(d=lever_width + 3, h=lever_thickness, center=true);
        }
        
        // Pivot hole (M4 bolt clearance) - oriented for vertical pivot axis
        translate([15, lever_thickness + 0.5, lever_width/2])
            rotate([90, 0, 0])
                cylinder(d=pivot_hole_diameter, h=lever_thickness + 1);
        
        // Connection hole through center of 25mm cylindrical extension (M4 bolt)
        translate([-10, lever_thickness + 0.5, lever_width/2])
            rotate([90, 0, 0])
                cylinder(d=mounting_hole_diameter, h=25 + 1);  // Goes through entire 25mm cylinder
    }
}
...
module assembly() {
    ...
    // Latch connector (attaches to lever's connection hole at 50mm from lever start)
    translate([10 - 15 + 50, 13, 12.5]) // Position at lever's connection hole (lever_length - 30 = 50mm)
...
module asa_printing_notes() {
    ...
    echo("Infill: 40-50% for outdoor durability and stress resistance");
    ...
    echo("CRITICAL: Ensure excellent first layer adhesion - bracket failure typically starts there");
}
