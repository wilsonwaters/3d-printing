// Gate Latch Lever v3 for 3D Printing (ASA Material)
...
//
// v3 changes from v2a:
//   - lever_thickness increased from 5mm to 7mm (broke just outside pivot in v2a)
//   - Pivot section stays 5mm × 5mm (Y × Z), same as handle
//   - Removed d=12 pivot reinforcement that bulged to 12mm in Z direction
//   - Flat bearing surface 7mm from pivot hole center (pivot_flat_diameter=14)
//   - Smooth ramp transition from 5mm pivot to 7mm arm to avoid stress concentration
//   - Truss redesigned with flat cubes — all within Z 0..lever_width (no bulge)

// Parameters
lever_length = 80;          // Total length of the lever arm
lever_width = 5;            // Width constrained to 5mm max (7mm clearance available)
lever_thickness = 5;        // Thickness matches handle; pivot pad provides reinforcement
handle_diameter = 15;       // Handle diameter
...
pivot_flat_diameter = 14;   // Flat bearing surface: 7mm from hole center
pivot_height = 5;           // Pivot section Y-height (same as handle)

// Derived
fudge = 0.01;
pivot_y_start = lever_thickness - pivot_height; // Where pivot section begins (flush with +Y face)
$fn = 32;

module gate_latch_lever() {
    difference() {
        union() {
            // Main lever arm — uniform 5mm thickness
            hull() {
                translate([-35, 0, 0])
            ...
            // Handle end (ergonomic grip — 15mm diameter pad)
            translate([lever_length - handle_diameter/2, lever_thickness/2, lever_width/2])
                rotate([90, 0, 0])
                    cylinder(d=handle_diameter, h=handle_thickness, center=true);

            // Pivot pad (same 15mm diameter as handle — reinforces bolt hole area)
            translate([15, lever_thickness/2, lever_width/2])
                rotate([90, 0, 0])
                    cylinder(d=handle_diameter, h=lever_thickness, center=true);

            // 25mm cylindrical extension at connection end for latch connector
            translate([-35, lever_thickness, lever_width/2])
                rotate([90, 0, 0])
                    cylinder(d=8, h=25);

            // Triangular truss — supports extension rod
            // Terminates at lever body edge (x=-35) so extension cylinder stays clean
            hull() {
                // Lever body connection
                translate([-5, 0, 0])
                    cube([1, lever_thickness, lever_width]);

                // Extension tip connection (at lever body edge, not past it)
                translate([-35, -12.5, 0])
                    cube([1, 1, lever_width]);
        ...
        // Pivot hole (M4 bolt clearance) — through pivot pad
        translate([15, lever_thickness + fudge, lever_width/2])
            rotate([90, 0, 0])
                cylinder(d=pivot_hole_diameter, h=lever_thickness + 2*fudge);

        // Connection hole through extension and truss (M4 bolt)
        translate([-35, lever_thickness + fudge, lever_width/2])
            rotate([90, 0, 0])
                cylinder(d=mounting_hole_diameter, h=40);
    }
}
...
module assembly() {
    ...
    // M4 bolt at lever-connector joint
...
module print_layout() {
    // Lever (print flat side down)
    translate([0, 0, lever_width])
        rotate([90, 180, 0])
            gate_latch_lever();
...
module asa_printing_notes() {
    ...
    echo("Layer Height: 0.2-0.25mm for strength");
    echo("Infill: 50-60% for structural strength");
    echo("Support: NO supports needed — designed support-free");
    ...
    echo("Hardware: M4 x 25mm bolt for pivot, self-tapping screws for mounting");
    echo("CRITICAL: Ensure excellent first layer adhesion");
    echo("v3 NOTES:");
    echo("- Lever arm 7mm thick (was 5mm) — stronger where v2a broke");
    echo("- Pivot section 5mm x 5mm — no bulge, matches handle");
    echo("- Smooth ramp from pivot (5mm) to arm (7mm) avoids stress concentration");
    echo("- Flat truss profile — all within 5mm lever width");
}

// Stress analysis recommendations
module stress_analysis_notes() {
    echo("=== STRESS POINTS TO MONITOR ===");
    echo("1. Transition zone from pivot (5mm) to arm (7mm) — smooth ramp reduces stress");
    echo("2. Pivot hole area - no sharp internal corners");
