...
assert(chamber_len_x*chamber_wid_y*(chamber_out_h-9) > tip_ml*1000*1.2,
       "chamber capacity must exceed tip volume by >=20%");
...
module funnel() {
    ...
    difference() {
        ...
        // knife edge: outside bevel rising to the sharp orifice edge
...
module bucket() {
    ...
    difference() {
        union() {
            translate([-bl/2, -bw/2, 0]) cube([bl, bw, chamber_out_h]);
            translate([-bl/2, -1.2, 0]) cube([bl, 2.4, divider_h]);
            // pivot boss slabs (rectangular: support-free, teardrop bore)
    ...
    // crank drive tab: fin on the right end wall, raised above the pivot tower;
    // the separate crank plate's pocket laps over it (transmits rotation)
