// === DESCRIPTION ===
// v4 - print-orientation audit after slicing the plate 3MF (2026-07-20):
//   1. DRUMS now print ring-gear-face DOWN (part= exports are pre-flipped):
//      v3 printed cam-end-down, perching the whole Dia46 digit rim on the
//      Dia15 cam/hub column with floating rings above. The internal web moved
//      next to the ring block (11 mm bridge), the carry-sector teeth gained a
//      45-deg underside taper, and the units drum gained a 45-deg support
//      cone between digit band and ratchet (stack grew 3.8 mm; the register
//      x-positions all derive from it).
//   2. PINIONS print sector-band-down with a full-Dia core everywhere except
//      the actual yoke/finger clearance window (45-deg cone rejoins it) - the
//      upper tooth bands no longer float off the slim waist.
//   3. The PAWL is now a separate flat-printed spring (pegs press into the
//      rocker arm) - in v3 its beam printed as a floating cantilever.
//   4. Slider button was dipping 1.9 mm below the bed -> flat-chinned.
//   5. Plate layout fits <=218 mm square so the auto-centred footprint clears
//      the Bambu X1/P1 18x28 mm front-left bed exclusion corner; 3MFs are
//      generated with X1C presets.
//   Remaining sub-1.5 mm ledges (lock-band seat stubs, ratchet tooth tips)
//   print with minor droop on non-working faces - accepted.
//
// v3 - refinements from inspecting the v2 bucket (2026-07-19):
//   1. divider crest now tapers to a point (the true "knife edge" that splits
//      the drip between the chambers - v2 left it a flat 2.4 mm wall top; the
//      v2 funnel-rim chamfer was a misread of this request, though kept)
//   2. crank drive tab centred on the pivot axis (y -9..+9, was -12..+6): its
//      small (~0.07 N.mm, ~2%) front-bias torque is now zero by symmetry.
//      Its position at one END of the axis never affected tipping (no moment
//      arm about the tip axis); it only loads that V-seat a fraction more.
//   The teardrop notch at the top of the pivot bore is INTENTIONAL: it is the
//   self-supporting crown for a horizontal printed hole; the pin rides the
//   bore bottom and never touches it.
//
...
//   drums     ring-gear face down (part= exports pre-flipped; digits on rim)
//   pinion    sector band down (part= export pre-flipped; full core, no float)
//   yoke/rocker/pawl/detent/hammer/slider/clip: flat
...
part = "assembly"; // ["assembly","mech","funnel","screen","body","base",
                   ...
                   //  "pinion","yoke","rocker","pawl","hammer","slider",
                   //  "detent","clip","plate_a","plate_b"]
...
units_cone_w = 3.8;   // 45-deg support cone between digit band and ratchet
ratchet_x0  = drum0_x + digit_band_w + 0.4 + units_cone_w;  // ratchet on units drum
...
echo(str("== chamber capacity at receiving tilt: ", recv_cap/1000,
         " mL vs ", tip_ml, " mL tip target"));
                ...
                union() {
                    ...
                    translate([-bl/2, 0, 0]) rotate([90,0,90]) linear_extrude(bl)
                        polygon([[-bw/2, 0], [bw/2, 0], [bw/2, lip_z],
                                 ...
                                 [-bw/2, lip_z]]);
                    // divider wall with a pointed crest (the knife edge that
                    // splits the spout drip cleanly between the two chambers)
                    translate([-bl/2, -1.2, 0]) cube([bl, 2.4, divider_h-5]);
                    translate([-bl/2, 0, 0]) rotate([90,0,90]) linear_extrude(bl)
                        polygon([[-1.2, divider_h-5-fudge], [1.2, divider_h-5-fudge],
                                 [0.25, divider_h], [-0.25, divider_h]]);
                    ...
                    // v3: centred on the pivot axis - zero tipping bias
                    translate([chamber_len_x/2+bucket_wall-fudge, -9, 0])
...
module crank_use() {
    ...
    difference() {
        union() {
            ...
            // v3: shifted +3 in y to follow the centred tab
            translate([tab_x-0.4, -10.6, 9])
...
module drum_common() {
    ...
    // web disc joins hub to rim, placed right against the ring-block end so
    // in the (flipped) print it bridges only ~11 mm onto the block below
    translate([0,0,ringb_z0-2.2]) tube(drum_od-2, hub_od-fudge, 2.2);
    ...
    // v4: teeth get a 45-deg underside taper (print is ring-face-down, so the
    // tooth depth grows from the digit-rim support outward - no floating).
    difference() {
        ...
        translate([0,0,sector_z0-fudge])
            cylinder(h=sector_band_w+2*fudge, r1=ring_tip_r-0.05, r2=19.0);
    }
    // digit band
...
module drum_units() {
    drum_common();
    translate([0,0,ringb_z0]) {
        // 45-deg support cone: carries the digit rim in the (flipped) print,
        // where the Dia40 ratchet alone couldn't support the Dia46 rim
        difference() {
            cylinder(h=units_cone_w, r1=drum_od/2, r2=ratchet_r_o+0.05);
            translate([0,0,-fudge])
                cylinder(h=units_cone_w+2*fudge, d=pin_d+tol_journal);
        }
        translate([0,0,units_cone_w]) {
            // ratchet, tooth tips tapered toward the track (print-underside)
            difference() {
                linear_extrude(ratchet_band_w) difference() {
                    ratchet_2d(); circle(d=pin_d+tol_journal); }
                difference() {
                    translate([0,0,-fudge]) cylinder(h=ratchet_band_w+2*fudge, r=25);
                    translate([0,0,-2*fudge]) cylinder(h=ratchet_band_w+4*fudge,
                        r1=ratchet_r_o+0.05, r2=track_r_o+0.05);
                }
            }
            // shallow 10-notch detent track (outboard of the ratchet): the
            // frame detent's nub rests in a notch at every digit position -
            // alignment + anti-reverse - at ~1/10 deep-ratchet climb torque
...
// print wrappers: drums print ring-gear-face DOWN (v4) - the mechanism-frame
// modules stay as the assembly uses them; only part= exports are flipped
module drum_std_print() {
    translate([0,0,drum_h]) rotate([180,0,0]) drum_std();
}
module drum_units_print() {
    translate([0,0,ringb_z0+units_cone_w+ratchet_band_w+track_band_w])
        rotate([180,0,0]) drum_units();
}
module track_2d() {
    difference() {
        circle(r=track_r_o);
...
// ---- TRANSFER PINION (printed sector-band-down via pinion_print()) ----
// v4: full-root-diameter core everywhere except the yoke/finger clearance
// window (slim Dia5.2 waist), with a 45-deg cone rejoining full diameter on
// the ring-band side. Printed sector-end-down, every tooth band sits on the
// core - nothing floats.
pin_waist_lo = (drum_x(0) - digit_z0) - pinion_x(0) - 0.3;   // clearance window
pin_waist_hi = pin_waist_lo + cam_band_w + 0.6;
module pinion() {
    difference() {
        union() {
            // tooth bands
            ...
            // core: full root Dia -> 45-deg cone -> waist -> full root Dia
            cylinder(r=pin_root_r-0.05, h=pin_waist_lo-2.3+fudge);
            translate([0,0,pin_waist_lo-2.3])
                cylinder(r1=pin_root_r-0.05, r2=sleeve_od/2, h=2.3+fudge);
            translate([0,0,pin_waist_lo])
                cylinder(d=sleeve_od, h=pin_waist_hi-pin_waist_lo+fudge);
            translate([0,0,pin_waist_hi])
                cylinder(r=pin_root_r-0.05, h=pinion_len-pin_waist_hi);
        }
        translate([0,0,-fudge]) cylinder(d=2.4, h=pinion_len+2*fudge); // lightening bore
    }
}
module pinion_print() { translate([0,0,pinion_len]) rotate([180,0,0]) pinion(); }
...
module yoke() {
    ...
    union() {
        ...
        // pivot stubs into the chassis yoke bosses (centre 2.1: bottom = bed)
        translate([-4+fudge, 0, 2.1]) rotate([0,90,0]) cylinder(d=4.2, h=4);
        translate([bar_l-fudge, 0, 2.1]) rotate([0,90,0]) cylinder(d=4.2, h=4);
...
module pawl_arm() {
    // v4: just the anchor pad with two press holes - the pawl spring itself
    // is now a separate flat-printed part (its beam printed as a floating
    // cantilever when it was integral). See pawl().
    rotate([0,0,180]) difference() {
        linear_extrude(rocker_t) hull() {
            translate([2,7]) circle(d=7);
            translate([19.5,25]) circle(d=8);
        }
        for (p=[[8,12.5],[16.5,21.5]]) translate([p[0], p[1], -fudge])
            cylinder(d=2.75, h=rocker_t+2*fudge);
    }
}

// ---- PAWL (separate flat spring; pegs press into the rocker anchor pad) ----
// Single-acting feed: a near-tangential flexure beam with a hook that dips
// below the ratchet crest circle (~2.7% strain at full tooth lift). Drive
// stroke: the hook bears on a tooth's radial face -> one digit. Return
// stroke: it flexes out and clicks over the crests; the saw form of the
// wheel sets the one-way direction. Printed flat (beam flexes in-plane);
// replaceable/tunable like the detent. The 2D geometry mirrors the rocker's
// anchor frame so that, mounted via rotate([0,90,0]), hook and holes land
// exactly where the v3 integral pawl sat.
pawl_x0 = ratchet_x0 + 1.8;   // lamina inner face (global x)
module pawl() {
    standoff = rocker_x0 - (pawl_x0 + 4);   // lamina top -> rocker plate face
    mirror([0,1,0]) {
        // lamina: base plate + flexure beam + hook
        linear_extrude(4) union() {
            hull() { translate([8,12.5]) circle(d=7.5);
                     translate([16.5,21.5]) circle(d=7.5);
                     translate([20, 24.3]) circle(d=6); }
            polygon([[3, 20.6], [20.5, 23.7], [20.5, 25.2], [3, 22.1]]);  // beam
            polygon([[2.6, 22.0], [5.6, 21.6], [5.1, 17.3], [3.3, 17.3]]); // hook
        }
        // standoff bosses + press pegs (into the rocker pad's 2.75 holes)
        for (p=[[8,12.5],[16.5,21.5]]) translate([p[0], p[1], 0]) {
            translate([0,0,4-fudge]) cylinder(d=5.8, h=standoff+fudge);
            translate([0,0,4+standoff-fudge]) cylinder(d=2.95, h=3.2+fudge);
        }
    }
}
module place_pawl() {
    translate([pawl_x0, drum_axis_y, fz + drum_axis_z])
        rotate([rocker_pose_deg, 0, 0]) rotate([0, 90, 0]) pawl();
}
...
module hammer() {
    ...
    union() {
        ...
        // pivot stubs into the journal-tower holes (centre 2.1: bottom = bed)
        translate([-4+fudge, 0, 2.1]) rotate([0,-90,0]) cylinder(d=4.2, h=4);
        translate([bar_l-fudge, 0, 2.1]) rotate([0,90,0]) cylinder(d=4.2, h=4);
        // snail-cam fingers
        for (n=[0:n_drums-1])
...
module slider() {
    ...
    union() {
        ...
        // button disc, flat-chinned so it doesn't dip below the bed (v4)
        difference() {
            translate([0, -2.8, bar_h/2]) rotate([-90,0,0]) cylinder(d=13.8, h=3+fudge);
            translate([-8, -6.5, -7]) cube([16, 8, 7.1]);
        }
...
module assembly() {
    ...
    color("BlueViolet")   place_pawl();
    color("SpringGreen")  place_detent();
    color("IndianRed")    place_hammer();
    color("Crimson")      place_slider();
}

// Plates: v4 splits the small parts over TWO plates, each well inside a
// 218 mm square, so Bambu's auto-centring keeps footprints clear of the
// X1/P1 18x28 front-left bed exclusion corner (see the skill's printer
// notes). plate_a = register (drums/gears - print at 0.12 mm if patient);
// plate_b = levers + screen.
module plate_a() {
    translate([26, 24, 0]) drum_units_print();
    translate([78, 24, 0]) drum_std_print();
    translate([26, 76, 0]) drum_std_print();
    translate([78, 76, 0]) drum_std_print();
    for (i=[0:2]) translate([114+i*22, 12, 0]) pinion_print();
    translate([140, 60, 0]) rocker();
    translate([176, 60, 0]) crank();
    translate([112, 30, 0]) detent();
    translate([185, 100, 0]) pawl();
    for (i=[0:2]) translate([110+i*13, 104, 0]) clip();
    for (i=[0:2]) translate([110+i*13, 118, 0]) clip();
}
module plate_b() {
    translate([46, 46, 0]) screen();
    translate([8, 112, 0]) yoke();
    translate([8, 145, 0]) hammer();
    translate([150, 20, 0]) slider();
}
...
module model() {
    ...
    else if (part=="drum_std")   drum_std_print();
    else if (part=="drum_units") drum_units_print();
    else if (part=="pinion")     pinion_print();
    ...
    else if (part=="plate_a")    plate_a();
    else if (part=="plate_b")    plate_b();
}
