...
//     which is FDM's strong direction, and the ~41cm^2 of total rib cross-section puts
//     the stress from a 25kg wet pot at ~0.06MPa — ~1000x below PETG yield, so no
//     creep in summer sun. It is also the lightest way to get this stiff (~123g).
...
//     seat plane to corral the pot. It needs stand_size >= pot_base_max + 4 or the lip
//     would foul the biggest pot's base, which an assert enforces.
//
// Terminology -> code:
...
//   "containment lip/corral"  -> rim_extra
...
//   Stronger / stiffer        -> lower cell_target, or rib_walls 4 -> 5
//   Add a lip that traps the  -> rim_extra = 8 AND stand_size = pot_base_max + 6
//     pot from sliding           (both, or the assert will stop the build)
...
// Layer Height: 0.2mm
// Walls/Perimeters: 4 (1.8mm) — the whole part IS perimeters
...
//   - Measured solid volume 96.9cm^3: ~123g in PETG, ~2-3h.
...
rib_walls   = 4;         // internal lattice ribs
outer_walls = 4;         // outer wall
...
n_cells    = max(3, 2 * floor(stand_size / cell_target / 2) + 1);
...
assert(cell_open >= 30,
       "centre cell smaller than 30mm - water from the drain hole needs a clear cell");
assert(n_cells >= 3,
...
assert(rim_extra == 0 || stand_size >= pot_base_max + 4,
       "a containment lip needs stand_size >= pot_base_max + 4, or it fouls the pot");
