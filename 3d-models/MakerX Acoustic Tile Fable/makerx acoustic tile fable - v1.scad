// === DESCRIPTION ===
// MakerX Skyline Acoustic Tile (Fable v1): a 250x250mm wall tile set that
...
logo_width  = 200;   // [mm] X-mark width on the logo tile
...
key_y       = 190;   // [mm] entry-hole center from bottom edge
...
// One skyline column (full cell footprint), hollowed if tall enough
module column(m, n) {
    s = s_val(m, n);
    h = col_h(s);
    cx = (m + 0.5) * cell_size;
    cy = (n + 0.5) * cell_size;
    if (s > 0) difference() {
        translate([m * cell_size, n * cell_size, base_thick - fudge])
            cube([cell_size, cell_size, h + fudge]);
